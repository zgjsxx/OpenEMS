...
int main(int argc, char* argv[]) {
  ...
  if (argc > 1) {
    ...
    if (first.find("config/tables") != std::string::npos || first.find(".csv") != std::string::npos) {
      OPENEMS_LOG_W("Collector", "CSV config path argument is ignored. Runtime config now loads from PostgreSQL only.");
  ...
  OPENEMS_LOG_I("Collector", "Loading runtime config from PostgreSQL");
